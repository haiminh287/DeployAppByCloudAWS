...
class HostService(Service):
    ...
    key_pair = Column(Text, nullable=True)
...
if __name__ == '__main__':
    with app.app_context():
        db.create_all()
        u = User(name='user', username='user', email='user2k4@gmail.com', password=str(
            hashlib.md5('123456'.encode('utf-8')).hexdigest()), user_role=UserRole.USER)
        db.session.add(u)
        ad = User(name='admin', username='admin', email='admin224@gmail.com', password=str(
            hashlib.md5('123456'.encode('utf-8')).hexdigest()), user_role=UserRole.ADMIN)
        db.session.add(ad)
        db.session.commit()
